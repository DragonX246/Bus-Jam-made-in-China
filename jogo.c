...
void jogar() {
    ...
    if (fdt == NULL) {
        printf("Nenhuma Fase Encontrada, pressione <enter> para voltar...\n");
        pausa();
        return;
    }
...
void configuracoes() {
    while (1) {
        ...
        if (scanf("%d", &menu) == 0) {
            ...
            char reinicia;
            scanf(" %c", &reinicia);
            while (reinicia != 'S' && reinicia != 's' && reinicia != 'N' && reinicia != 'n') {
                scanf(" %c", &reinicia);
            }
            if (reinicia == 'S' || reinicia == 's') {
                reiniciarRanking();
                printf("\nRanking zerado! Tecle <enter> para continuar");
                pausa();
            }
        } else if (menu == 2) {
            system(CLEAR);
